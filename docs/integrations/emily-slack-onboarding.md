...
2. Set on the API host (via the server environment path used by
   `floom-api.service`, then restart — never paste tokens into chat):
...
3. In the Slack app config set Event Subscriptions / Interactivity / `/floom`
   ...
   Floom echoes the `challenge` only after validating the HMAC signature.
...
2. The link opens `<frontend>/settings?slack_claim=<token>`; logging in there
   ...
   the logged-in Floom user. Pending claims expire and are replaced on
   re-request; active bindings are never silently replaced.
3. From then on, DMs/@mentions from that Slack user run as the bound Floom
...
- **@mention in a channel**: `/invite @Floom` to the channel, then
  `@Floom <request>`. Replies land in the message's thread.
