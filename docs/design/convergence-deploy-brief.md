...
3. Deploy: Cloud dashboard (web/ context, vercel --prod via token, project workeros-cloud-dashboard) + bump/restart the Cloud API (workeros-cloud-api) if the engine moved for it; OS frontend auto-deploys on main (verify) + deploy OS API via /opt/workeros-api-deploy/ops/deploy-api.sh + clean restart if backend changed.
