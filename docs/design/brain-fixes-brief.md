...
the operator live-walk. The brain (context packs + files) is broken and unintuitive. Engine = source of truth; the Cloud has its own context routes (workeros-cloud/apps/api/routes/context_previews.py) + Supabase repos. Fix in engine where the logic is shared; cloud-only where it is a cloud repo/route. Investigate ROOT CAUSE before patching. Do NOT prod-deploy without ops/smoke-routes.sh.
...
Clicking download on a brain file returns detail "Context not found". Trace the download endpoint (apps/api/main.py + workeros-cloud context_previews.py). The context/file exists (it is listed) but the download lookup misses it — likely id/slug/workspace-scoping mismatch (same class as the M74/M75 stale-workspace bug) OR the download route resolves the wrong id. Fix so download returns the file. Verify live.
...
The brain/context is currently read-only. the operator wants to add/edit files (write). Add the write path: upload/create/edit/delete files in a pack, with the proper API + UI. (Pairs with M85c.)
