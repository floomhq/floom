...
- The footer of the sidebar shows "Floom v0" + a small `Light` / `Dark` toggle pill. The pill duplicates the Settings → Appearance tab. Either remove the inline toggle (since Appearance handles it) or remove the duplicate tab.
...
**P1 (uncomfortable):**
- This is a **security-relevant page** (authorizing a CLI to act on the user's behalf), but it's rendered inside the standard app chrome with the sidebar, "Floom v0" footer, and theme toggle — same as every other page. OAuth/CLI-auth pages should feel distinct: removed sidebar, centred card, no other navigation. Currently a careless user sees the sidebar nav items and clicks away from the consent screen entirely.
...
- "Floom v0" footer text is in every sidebar — every page. Once Floom hits v1, that string is stale. Replace with semantic version from the build (`v0.1.0` is already shown on /settings/system).
