...
**Method:** LIVE prod verification. self-hosted server IP passes the Cloudflare edge with `x-floom-secret`
...
Walked via self-hosted server Browser Broker (lease pool-e, released). CDP-driven viewport control; per-surface
...
- self-hosted server IP passes the CF edge with `x-floom-secret` + a non-bot UA; default bot UAs are blocked
