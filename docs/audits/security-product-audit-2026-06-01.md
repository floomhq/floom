...
- Live checks against `workers.floom.dev`, `workers-api.floom.dev`, `workeros.floom.dev`, and `workeros-api.floom.dev` from self-hosted server.
...
|---|---|
| Web frontend has 43 Turbopack module errors. | Stale. `npm run build` passed locally before this patch on self-hosted server. Independent Codex build failed only in a read-only/offline temp copy because Google Fonts fetch was blocked. |
...
| `workers-api.floom.dev` unauthenticated health/preflight blocked by Cloudflare. | Still observed from self-hosted server. This is an edge/WAF behavior, not FastAPI code behavior; it blocks anonymous probes before the API. |
