...
- **API:** `/root/workeros/apps/api/main.py` (6195 lines, 63 endpoints, FastAPI + SQLite).
  ...
  Service: `systemd workeros-api.service` on self-hosted server, port 8011, ProxyPass via Cloudflare to `workers-api.floom.dev`.
...
- **Composio:** per-call `user_id="federico"` hardcoded; multi-tenant work belongs in the -cloud lane (see `hosted builds` + the cloud briefing at `/tmp/managed-deployment-briefing.txt`).
...
- **Composio multi-tenancy**: out of scope. That's the `managed-deployment` lane (see `/tmp/managed-deployment-briefing.txt`).
