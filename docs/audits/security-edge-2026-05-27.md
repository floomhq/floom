...
### P2: Cloudflare WAF blocks /health from AX41 IPv6 — NOT FIXED

`GET https://workers-api.floom.dev/health` from AX41 (`2a01:4f9:3b:432a::2`) still returns CF 403. Internal `127.0.0.1:8011/health` returns 200 as expected.
...
| A3: /health without secret | 200 (internal) | PASS; CF WAF still blocks AX41 IPv6 externally |
...
| P2: CF WAF blocks /health from AX41 | OPEN | STILL OPEN | 0 |
...
5. **P2 (low):** Cloudflare WAF: add bypass rule for `/health` and `/healthz` paths, or whitelist AX41 egress IPv6 `2a01:4f9:3b:432a::2`.
