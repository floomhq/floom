...
- Run the test from inside the self-hosted server → uvicorn local-loopback path with the right header set.
...
- GitHub Actions workflow (`.github/workflows/deploy-api.yml`) that SSHes into self-hosted server on merge to main and runs the pull+restart
