...
| 4 | **P2** | Cloudflare WAF blocks `/health` and `/healthz` from AX41's IPv6 | `curl https://workers-api.floom.dev/health` from AX41 → 403 (CF block page). Internal `curl http://127.0.0.1:8011/health` → 200 OK. The WAF fires on unauthenticated requests from AX41's IPv6 block (`2a01:4f9:3b:432a::2`), which means external monitoring/LB health checks will fail. | Whitelist AX41 IPv6 or add a Cloudflare Page Rule to bypass WAF for `/health` and `/healthz` paths |
...
For Cloudflare WAF: add a WAF bypass rule for `/health` and `/healthz` paths (or whitelist AX41's egress IP in CF).
