...
- Direct live API upload to an existing pack returned 200 before the patch. Upload with `create_if_missing=true` returned 200 against the patched self-hosted server production service on `127.0.0.1:8030`.
...
- Patched self-hosted server service accepted live `create_if_missing` upload on `127.0.0.1:8030`.
