...
### API + Endpoints
- `/healthz` and `/health` return `{"status":"ok"}` 200 when called with secret (app exemption works; Cloudflare WAF blocks self-hosted server IPv6 for unauthenticated calls — not a functional issue)
