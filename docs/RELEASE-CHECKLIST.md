...
- **Security** — P0/P1 (platform-secret override, `_upsert_env_var` newline injection) verified patched; 9 security issues fixed + closed; no live secrets and no current infra in git history (self-hosted server stale), so history is safe to publish as-is.
