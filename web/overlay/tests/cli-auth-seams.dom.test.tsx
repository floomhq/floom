...
describe("CLI auth seams", () => {
  ...
  it("keeps the OSS endpoint and client name by default", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Authorize CLI" })).toBeInTheDocument();
    expect(screen.getByText("Floom")).toBeInTheDocument();
    expect(screen.getByText("Confirm this code to connect the Floom CLI to your account.")).toBeInTheDocument();
    expect(screen.getByLabelText("Confirmation code")).toHaveTextContent("ABCD-2345");
    expect(screen.getByRole("button", { name: "Approve" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Deny" })).toBeInTheDocument();
    expect(screen.getByText("Details")).toBeInTheDocument();
    expect(screen.getByText("Caution:")).toBeInTheDocument();
    expect(screen.queryByText("Connect your agents")).toBeNull();
