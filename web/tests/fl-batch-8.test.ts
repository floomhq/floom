...
function test556ConnectionsSetupCallout(): void {
  ...
  assert(s.includes("missingBySlug"),
    "ConnectionsClient must track missingBySlug state");
  assert(s.includes("computeMissingBySlug"),
    "ConnectionsClient must define computeMissingBySlug helper");
  assert(s.includes("Setup required"),
    "ConnectionsClient must render a 'Setup required' callout");
