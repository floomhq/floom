...
export const api = {
  ...
  workers: {
    ...
    run: async (id: string, inputs: Record<string, unknown>) => {
      ...
      captureProductEvent("run_started", {
        worker_id: id,
  ...
  system: {
    ...
    listWorkspaceChangelog: (limit = 50) =>
      fetchJson<import("@/lib/types").ChangelogEntry[]>(`/workspace/changelog?limit=${limit}`),
  ...
  connections: {
    ...
    initiate: async (app_name: string) => {
      ...
      captureProductEvent("connection_added", {
