...
- Node base: `2` CPU, `1024` MB memory.
...
E2B SDK 2.28.0 fixes CPU and memory on the template; `Sandbox.create()` does not
accept per-create resource arguments. Workers can request a larger template with
`resources.memory_mb` / `resources.cpu_count` or `exec.resources.*`. Register the
matching template ID or alias with one of these env vars:
