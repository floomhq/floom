#!/usr/bin/env python3
"""
ops/verify-schema.py — schema drift detector for Floom.
...
Usage:
    ...
    db_path defaults to /opt/floom/data/floom.db (or $FLOOM_DB).
...
def main() -> int:
    ...
    if len(sys.argv) >= 2:
        ...
        if not db_path:
            # Try the standard relative path from /opt/floom/apps/api working dir
            candidates = [
                "/opt/floom/data/floom.db",
