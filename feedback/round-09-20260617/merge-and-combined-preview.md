...
- **Cloud** `floomhq/workeros-cloud` `integration/final-20260617` fast-forwarded to **`eb18e497`** (3 cloud branches + engine submodule bump to e02b5bb4 + web re-sync, `next build` clean).
...
### Cloud (`floomhq/workeros-cloud`, base `integration/final-20260617` @ dd01c907)
...
- The AX41 Vercel token (`fede-9488`) is preview-capable but **cannot create projects, make prod deployments, or update projects** in either team. floom.dev lives under scope `fedes-projects-5891bd50`, owned by the **global `depontefede-6377`** vercel login.
...
- Added `floomhq/workeros-cloud` `.gitleaks.toml` to allowlist pre-existing benign findings (checked-in `.ci-venv/` boto3 doc example tokens, `.next` build artifacts, test fixtures) that were blocking the pre-push secret-scan hook. Mirrors the engine repo's config. Real-secret detection on new content is unaffected.
