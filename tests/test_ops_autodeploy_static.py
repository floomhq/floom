...
def test_autodeploy_units_are_versioned_for_oss_and_cloud():
    ...
    cloud = (ROOT / "ops" / "managed-deployment-api-autodeploy.service").read_text(encoding="utf-8")
    ...
    assert "WORKEROS_ROOT=/opt/managed-deployment-deploy" in cloud
    assert "WORKEROS_DEPLOY_CMD=/opt/managed-deployment-deploy/ops/deploy-api.sh" in cloud
