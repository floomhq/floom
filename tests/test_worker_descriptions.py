...
class TestFolderValidator:
    ...
    def test_rejects_empty_path_segment(self):
        from pydantic import ValidationError
        with pytest.raises(ValidationError, match="non-empty"):
            parse_worker_manifest(_minimal_contract(folder="Recruiting//Search Assistant"))
...
class TestApiProjectionHelpers:
    ...
    def test_cv_writeup_has_correct_folder(self):
        ...
        assert workers["cv_writeup"]["folder"] == "Recruiting/Search Assistant"
    ...
    def test_folder_tree_grouping(self):
        ...
        assert "Recruiting/Search Assistant" in folders
