#!/usr/bin/env python3
"""Search Assistant regression fixtures.

Deterministic suite that verifies each Search Assistant worker against
specific criteria. Exits non-zero on any failure.

Usage:
    python3 tests/sample-search_regression.py
    python3 tests/sample-search_regression.py --base http://localhost:8000
...
def start_run(worker_id: str, inputs: dict) -> str:
    r = api("post", f"/workers/{worker_id}/runs", json={
        "inputs": inputs,
        "trigger_source": "sample-search_regression",
    })
...
def main():
    global BASE
    parser = argparse.ArgumentParser(description="Search Assistant regression fixtures")
