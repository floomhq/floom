...
class TestFolderTreeGrouping:
    ...
    def _workers(self):
        return [
            {"id": "cv_writeup", "folder": "Recruiting/Search Assistant"},
            {"id": "reverse_match_crm", "folder": "Recruiting/Search Assistant"},
    ...
    def test_nested_folders_are_children(self):
        ...
        assert child_names == {"Compliance", "Search Assistant"}
    ...
    def test_count_aggregates_all_workers_under_path(self):
        ...
        nova = next(c for c in recruiting["children"] if c["name"] == "Search Assistant")
        assert nova["count"] == 2
...
class TestTagChipFiltering:
    def _workers(self):
        return [
            {"id": "cv_writeup", "tags": ["recruiting", "cv", "sample-search"], "folder": "Recruiting/Search Assistant"},
    ...
    def test_folder_filter_top_level_includes_children(self):
        """Filtering on 'Recruiting' includes Recruiting/Search Assistant workers."""
        result = filter_workers(self._workers(), None, "Recruiting")
        assert len(result) == 1
        assert result[0]["id"] == "cv_writeup"

    def test_folder_filter_exact_subfolder(self):
        result = filter_workers(self._workers(), None, "Recruiting/Search Assistant")
        assert len(result) == 1
