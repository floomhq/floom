...
6. **workeros-mcp — primary agent interface (NEW T1f, Federico 2026-05-26)** — external agents (Claude, Cursor, ChatGPT, etc.) discover + manage + run workers via MCP. Federico: "agents need to be able to work with workeros — especially creating new workers, crons, etc. Should be super simple for them." Wedge: workeros = AI-native n8n; the AI-native surface IS MCP, not REST. MVP tools: `workers.list`, `workers.get`, `workers.create` (accepts WorkerContract YAML), `workers.update` (trigger + cron + inputs + grants), `workers.delete`, `workers.run` (ad-hoc), `runs.list`, `runs.get`, `runs.watch`. Auth = same `x-floom-secret` as REST, transported via MCP server config. Deferred from MVP: connections OAuth (needs human handoff), secret rotation (low frequency, frontend OK), skill-library browse (T2 once skills.floom.dev integration lands). Distribution: `npx @floomhq/workeros-mcp` + a one-line install snippet on `/workers` empty state.
...
| Hosting | Vercel (frontend), AX41 systemd (API) | Set up earlier this session |
