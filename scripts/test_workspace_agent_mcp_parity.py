...
EXPECTED_TOOL = "ask_workeros_workspace_agent"
...
def assert_discovery() -> None:
    ...
    assert payload["name"] == "workeros-workspace-agent", payload
    assert payload["protocol"] == "2024-11-05", payload
    assert payload["transport"] == "streamable-http", payload
    assert EXPECTED_TOOL in payload["tools"], payload
    print(f"PASS discovery: {payload['endpoint']} exposes {EXPECTED_TOOL}")


def assert_initialize() -> None:
    result = rpc("initialize", rpc_id=10)
    assert result["serverInfo"]["name"] == "workeros-workspace-agent", result
...
def assert_tools_list() -> None:
    ...
    print("PASS tools/list: workspace-agent tool schema returned")
