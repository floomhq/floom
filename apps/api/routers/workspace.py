...
def put_workspace_setting(
    ...
    # UNION + SEMANTIC RECONCILIATION FLAG (round-09 #1127/#1314 base vs main):
    # main's _validate_workspace_setting is the comprehensive validator (read-only
    # keys, allowed keys, bools, approval_default, failure_email_to, spend cap,
    # max_output_tokens) and must run (it produces `value`, used in the INSERT).
    # BUT main allows default_timeout_seconds up to 86400s while the runtime
    # ceiling MAX_RUN_TIMEOUT_SECONDS is 3600s, so the DB could hold a value the
    # runtime silently truncates — the exact bug base #1127/#1314 guarded against.
    # We layer base's stricter runtime-ceiling check ON TOP so writes stay
    # consistent with what run dispatch will actually honor.
    # >>> HUMAN REVIEW: confirm the intended max for default_timeout_seconds is the
    #     runtime ceiling (3600) and not main's 24h — this changes the API contract.
    ...
    if key == "default_timeout_seconds":
        ...
        try:
            validate_default_timeout_seconds(int(float(value)))
