...
async def _cloud_engine_auth_for_request(request: Request) -> Any:
    from apps.api.auth.supabase_provider import SupabaseAuthProvider
    from auth.context import AuthContext as _AuthContext
...
@app.post("/api/workers/{worker_id}/share-link")
async def cloud_create_worker_share_link(worker_id: str, request: Request) -> Any:
    """Cloud route for engine standalone worker share links.

    Without this wrapper, Cloud's worker detail override shadows the mounted
    engine route and returns 404 for POST /workers/{id}/share-link.
    """
    import asyncio as _asyncio

    engine_auth = await _cloud_engine_auth_for_request(request)
    repos = engine_main.get_repositories()
    return await _asyncio.to_thread(
        engine_main.create_worker_share_link,
        worker_id,
        engine_auth,
        repos,
    )
...
@app.post("/api/contexts/{name}/share-link")
async def cloud_create_brain_pack_share_link(name: str, request: Request) -> Any:
    import asyncio as _asyncio

    engine_auth = await _cloud_engine_auth_for_request(request)
    repos = engine_main.get_repositories()
    return await _asyncio.to_thread(
        engine_main.create_brain_pack_share_link,
        name,
        engine_auth,
        repos,
    )
...
@app.post("/api/contexts/{name}/files/{file_path:path}/share-link")
async def cloud_create_brain_file_share_link(name: str, file_path: str, request: Request) -> Any:
    import asyncio as _asyncio

    engine_auth = await _cloud_engine_auth_for_request(request)
    repos = engine_main.get_repositories()
    return await _asyncio.to_thread(
        engine_main.create_brain_file_share_link,
        name,
        file_path,
        engine_auth,
        repos,
