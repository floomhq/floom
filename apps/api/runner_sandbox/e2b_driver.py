"""E2B cloud sandbox driver for Workeros.
...
class E2BSandboxDriver(SandboxDriver):
    """Runs worker code in an E2B cloud sandbox.

    The worker's run.py MUST:
    1. Read inputs from inputs.json
    2. Optionally read secrets from secrets.json (or env vars)
    ...
    def _run_in_sandbox(
        ...
        sandbox = Sandbox(api_key=api_key, timeout=max(timeout_seconds + 60, 180))

        try:
            workdir = "/home/user/worker"
            sandbox.filesystem.make_dir(workdir)
            ...
            if req_path.exists() and req_path.read_text().strip():
                ...
                if install_result.exit_code != 0:
                    err = f"pip install failed (exit {install_result.exit_code}): {(install_result.stderr or '')[:500]}"
            ...
            run_cmd = f"cd {workdir} && python run.py"
            ...
            # Read result.json
            try:
                result_raw = sandbox.filesystem.read(f"{workdir}/result.json")
            ...
            try:
                sandbox.close()
                log_fn("[e2b] Sandbox closed", "debug")
            except Exception as close_exc:
                logger.warning("Failed to close E2B sandbox: %s", close_exc)
