"""Smoke tests for Search Assistant Review Pack public routes."""
...
def test_review_pack_public_vote_roundtrip(monkeypatch, tmp_path):
    ...
    pack_id = "rp_sample-customer_gmbh_2026-06-22"
    rel = f"review-packs/{pack_id}/pack.json"
    pack = {
        "schema_version": "1.0",
        "id": pack_id,
        "client": {"slug": "sample-customer-gmbh", "name": "sample-customer GmbH"},
        "meta": {
            "title": "t",
    ...
    with TestClient(main.app) as client:
        created = client.post("/contexts/sample-search-sample-customer", json={"writeable": True}, headers=_secret_headers())
        assert created.status_code == 200, created.text

        encoded_rel = "/".join(quote(part) for part in rel.split("/"))
        put = client.put(
            f"/contexts/sample-search-sample-customer/files/{encoded_rel}",
        ...
        mint = client.post(
            f"/contexts/sample-search-sample-customer/review-packs/{pack_id}/share-link",
            headers=_secret_headers(),
        )
    ...
    feedback_files = list((contexts_dir / "sample-search-sample-customer" / "feedback" / "review" / pack_id).glob("*.json"))
