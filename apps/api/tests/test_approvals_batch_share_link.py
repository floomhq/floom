...
def test_cloud_batch_link_uses_active_workspace_context(client_and_main, monkeypatch):
    client, main = client_and_main
    from auth.factory import get_auth_provider, register_auth_provider
    from db.factory import register_repositories
    import git_ops

    auth_provider = get_auth_provider()
    repos = main.get_repositories()
    register_auth_provider("cloud", lambda: auth_provider)
    register_repositories("cloud", lambda: repos)
    monkeypatch.setenv("WORKEROS_DEPLOY", "cloud")
    git_ops.set_workspace_id_resolver(lambda: "ws_cloud_batch")
    try:
        _seed_approval(
            main,
        ...
        assert response.status_code == 200, response.text
        assert [item["id"] for item in response.json()["approvals"]] == ["apr_cloud"]

        mismatch = client.post(
            "/approvals/batch-share-link",
            headers={"x-floom-workspace": "ws_other"},
        )
