...
def test_error_raw_preserves_message_but_no_path() -> None:
    ...
    assert "/opt/floom" not in error_raw
...
def test_log_message_strips_host_path() -> None:
    msg = "wrote /opt/floom/data/artifacts/run_x/out/sorted.csv"
    redacted = main._redact_public_log_message(msg)
    assert "/opt/floom" not in redacted
...
def test_sse_error_field_maps_to_calm_headline() -> None:
    ...
    for raw, code in cases:
        ...
        for bad in ["~~~", "^~", "/home/user", "/opt/floom", "Traceback", "ZeroDivision", "unsupported operand", "Command exited"]:
