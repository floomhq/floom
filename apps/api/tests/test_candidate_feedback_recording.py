...
def test_concurrent_record_candidate_feedback_calls_create_distinct_files(monkeypatch, tmp_path):
    ...
    with TestClient(main.app) as client:
        created = client.post("/contexts/sample-search-data", json={"writeable": True}, headers=_secret_headers())
        ...
        def _post(payload):
            return client.post(
                "/contexts/sample-search-data/record-candidate-feedback",
                json=payload,
    ...
    files = sorted((contexts_dir / "sample-search-data" / "feedback" / "raw").glob("*/*.json"))
    assert len(files) == 2
...
def test_mcp_record_candidate_feedback_writes_event_file(monkeypatch, tmp_path):
    ...
    stored_path = contexts_dir / "sample-search-data" / record["path"]
