...
_DEFAULT_RUN_PY_STUB = (
    ...
    "    # Script workers read inputs.json and write result.json in the working\n"
    "    # directory. Edit this placeholder to produce real outputs.\n"
    "    inputs_path = Path(\"inputs.json\")\n"
    "    inputs = json.loads(inputs_path.read_text(encoding=\"utf-8\")) if inputs_path.exists() else {}\n"
    "    _ = inputs\n"
    "    Path(\"result.json\").write_text(\n"
    "        json.dumps({\"status\": \"success\", \"outputs\": {}, \"artifacts\": [], \"error\": None}),\n"
    "        encoding='utf-8',\n"
    "    )\n"
)
...
def _worker_public_token(worker: Dict[str, Any]) -> str:
    ...
    secret = (
        os.environ.get("WORKEROS_WORKER_PUBLIC_SIGNING_SECRET")
...
def _worker_detail_artifact_previews(
    *,
    ...
    from services.run_access import _is_sensitive_artifact_row
...
def _make_worker_detail_last_run(
    ...
    *,
    ...
    from models import DetailLastRun
...
def _build_worker_detail(
    ...
    *,
    ...
    # capability, so only surface it to callers who can actually run the worker
