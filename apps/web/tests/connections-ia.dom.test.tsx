...
vi.mock("@/lib/api", () => ({
  api: {
    ...
    connections: {
      list: vi.fn().mockResolvedValue([CONNECTED_FIXTURE]),
      delete: vi.fn(),
      test: vi.fn(),
      peek: vi.fn().mockResolvedValue({ emails: [] }),
      activity: vi.fn().mockResolvedValue([]),
      tools: vi.fn().mockResolvedValue({ tools: [] }),
      toolPresets: vi.fn().mockResolvedValue({ tools: [] }),
    },
...
describe("Connections IA — renamed, no count chips, standard filters, Browse=Add", () => {
  ...
  it("exposes Connected / MCP / Secrets through the standard `filters` affordance, no Browse chip", async () => {
    ...
    const filterToggle = await screen.findByRole("button", { name: /add filter/i });
