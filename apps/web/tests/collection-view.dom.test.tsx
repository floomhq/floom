...
function makeConfig(over: Partial<CollectionConfig<Item>> = {}): CollectionConfig<Item> {
  return {
    ...
    detail: (i) => ({
      ...
      tabs: [
        { key: "About", label: "About", custom: "settings-form" as const, render: () => <div>About {i.name}</div> },
        { key: "Runs", label: "Runs", custom: "run-output" as const, render: () => <div>Runs body</div> },
      ],
    }),
