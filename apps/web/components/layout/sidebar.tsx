...
export function SidebarPrimaryActions({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <div className="px-3 pb-3 pt-3 space-y-1.5">      {/* #902 (wireframe newbtn): creating a worker = a conversation with
      ...
      <button
        type="button"
        onClick={onSearch}
        className="flex h-8 w-full items-center gap-2 rounded-[var(--radius-ui)] bg-transparent px-2.5 text-sm text-[var(--ink-mute)] hover:bg-[var(--bg-3)] hover:text-ink transition-colors duration-150"        aria-label="Open command palette"
      >
        ...
        <span className="ml-auto inline-flex items-center gap-0.5 text-[10px] tracking-widest text-[var(--ink-faint)]">
          ...
          <kbd className="rounded-[var(--radius-ui)] bg-[var(--bg-3)] px-1 py-0.5 font-mono">K</kbd>        </span>
      </button>
    </div>
  );
}
