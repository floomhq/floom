          ...
          {actions.map((action) =>
            action.method === "GET" ? (
              <a
                key={action.id}
                href={action.href}
                className="inline-flex items-center gap-1 rounded-[var(--radius-ui)] bg-background px-2 py-1 text-[11px] font-medium text-foreground hover:bg-[var(--active-nav-bg)] transition-colors"
              >
              ...
              <span
                key={action.id}
                className="inline-flex items-center rounded-[var(--radius-ui)] bg-background px-2 py-1 text-[11px] text-muted-foreground"
              >
                {action.label ?? action.id}
