      ...
      >
        {segments.map((seg, i) =>
          seg.highlighted ? (
            <mark
              key={i}
              className="rounded-[3px] bg-amber-200/70 px-0.5 text-[var(--ink)] dark:bg-amber-500/30"
            >
              {seg.text}
            </mark>
          ) : (
      ...
      <div className="flex flex-col gap-3">
        <div className="rounded-[var(--radius-button)] [border:var(--bd-card)] bg-[var(--paper)] p-3">
          ...
          <button
            ...
            className="mt-2 inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-[var(--radius-button)] bg-[var(--primary)] px-3 text-xs font-medium text-[var(--primary-text)] disabled:opacity-40"
          >
