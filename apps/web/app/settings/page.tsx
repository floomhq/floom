...
function getSelectionFromLocation(): { section: SectionKey; systemTab: SystemSubTabKey } {
  if (typeof window === "undefined") return { section: "system", systemTab: "info" };
  const params = new URLSearchParams(window.location.search);
  const selParam = params.get("sel");
  const fromQuery = sectionFromCandidate(selParam || params.get("tab"));
  const fromHash = sectionFromCandidate(window.location.hash.replace(/^#/, ""));
  const section = fromQuery || fromHash || "system";
  const tabParam = selParam ? params.get("tab") : null;
  const systemTab = isValidSubTab(section, tabParam) ? (tabParam as SystemSubTabKey) : "info";
...
function SettingsContent() {
  ...
  useEffect(() => {
    function syncFromLocation() {
      ...
      if (nextSel) {
        ...
        const tabParam = selParam ? params.get("tab") : null;
        const nextTab = isValidSubTab(nextSel, tabParam) ? (tabParam as SystemSubTabKey) : "info";
        setActiveSection(nextSel);
        setActiveSystemTab(nextTab);
      }
      setSearch(window.location.search);
    }
    syncFromLocation();
  ...
  function writeSelectionToUrl(nextSel: SectionKey, nextTab: SystemSubTabKey | null) {
    const params = new URLSearchParams(searchParams.toString());
    params.set("sel", nextSel);
    if (nextSel === "system" && nextTab && isValidSubTab(nextSel, nextTab)) params.set("tab", nextTab);
  ...
  function handleSectionChange(value: string) {
    ...
    writeSelectionToUrl(value, value === "system" ? activeSystemTab : null);
  }

  function handleSystemTabChange(value: string) {
    if (!isValidSubTab("system", value)) return;
    const nextTab = value as SystemSubTabKey;
    setActiveSystemTab(nextTab);
    writeSelectionToUrl("system", nextTab);
  }
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col gap-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Workspace settings and your account settings, kept separate.
        </p>
      </div>
...
function BehaviourSettings({ canEdit = true }: { canEdit?: boolean }) {
...
function WorkspaceInfoSettings({ canEdit = true }: { canEdit?: boolean }) {
  const [values, setValues] = useState<Record<string, string> | null>(null);
...
function ModelDefaults({ canEdit = true }: { canEdit?: boolean }) {
  const [values, setValues] = useState<Record<string, string> | null>(null);
