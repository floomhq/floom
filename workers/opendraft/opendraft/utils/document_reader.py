"""Read documents (PDF, markdown, text) into plain text."""
...
def read_document(file_path: Path, max_chars: Optional[int] = None) -> str:
    ...
    if max_chars and len(text) > max_chars:
        text = text[:max_chars]
        logger.warning("Truncated document to %s characters", max_chars)
...
def _read_markdown(file_path: Path) -> str:
    """Read markdown file, optionally stripping formatting."""
    text = file_path.read_text(encoding="utf-8")
    # Keep markdown as-is for LLM processing
    # The LLM can understand markdown structure
    return text
