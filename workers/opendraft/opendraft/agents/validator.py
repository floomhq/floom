"""Validator agent: verifies citations, checks consistency, fact-checks claims."""
...
class ValidatorAgent(BaseCodeAgent):
    """
    Replaces v1's Skeptic + Verifier + Referee.

    Runs DOI verification, citation audit via run_code (local sandbox),
    consistency checks.
    """
    ...
    def __init__(self, state: SharedState, model_name: str = "gemini-3-flash-preview"):
        ...
        # Wrapper that reads draft from workspace before auditing
        def audit_draft_citations(draft_filename: str = "draft.md"):
            draft_text = workspace_ops.read_file(draft_filename)
            if draft_text.startswith("File '"):
                return {"error": f"Could not read {draft_filename}"}
            return citation_ops.audit_draft_citations(draft_text)

        function_handlers = {
            "verify_doi_batch": citation_ops.verify_doi_batch,
            ...
            "validate_citation_formats": citation_ops.validate_citation_formats,
        ...
        system_prompt = BaseCodeAgent.load_prompt("validator")
